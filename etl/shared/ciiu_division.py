...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        df = df[['section_name', 'division_name', 'section_id', 'division_id']]

        additional_df = pd.DataFrame(data=[['No determinado', 'No determinado', 'Z', '00']], columns=df.columns)

        df = df.append(additional_df)
        df.drop_duplicates(subset=['division_id'], inplace = True)
        
        return df

class CIIU_Division_Pipeline(EasyPipeline):
    @staticmethod
    def steps(params):
        ...
        load_step = LoadStep("dim_shared_ciiu_division", db_connector, if_exists="drop", pk=["division_id"], dtype=dtype)

        return [transform_step, load_step]

def run_pipeline(params: dict):
    pp = CIIU_Division_Pipeline()
    pp.run(params)
