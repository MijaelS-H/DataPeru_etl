...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        df = df[['section_name', 'division_name', 'group_name', 'section_id', 'division_id', 'group_id']]

        additional_df = pd.DataFrame(data=[['No determinado', 'No determinado', 'No determinado', 'Z', '00', '000']], columns=df.columns)
...
class CIIU_Group_Pipeline(EasyPipeline):
    @staticmethod
    def steps(params):
        ...
        load_step = LoadStep("dim_shared_ciiu_group", db_connector, if_exists="drop", pk=["group_id"], dtype=dtype)

        return [transform_step, load_step]

def run_pipeline(params: dict):
    pp = CIIU_Group_Pipeline()
    pp.run(params)
