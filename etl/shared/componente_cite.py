...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        df = pd.DataFrame({"componente_id": list(range(len(componente_list))),"componente": sorted(componente_list)})
        
  
        return df

class ComponentePipeline(EasyPipeline):
    @staticmethod
    def parameter_list():
        return [
    ...
    def steps(params):
        ...
        load_step = LoadStep(
            "dim_shared_cite_componente", db_connector, if_exists="drop", pk=["componente_id"], dtype=dtype)
...
if __name__ == "__main__":
    ...
    componente_pipeline = ComponentePipeline()
    componente_pipeline.run(
        {
