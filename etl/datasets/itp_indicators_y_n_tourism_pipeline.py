...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        df1_1 = pd.read_excel(path.join(params["datasets"], "A_Economia", "A.147.xls"), sheet_name = "19.2 (c)", skiprows = (0,1,2,3), usecols = "A:K")[3:44]
        df1_2 = pd.read_excel(path.join(params["datasets"], "A_Economia", "A.147.xls"), sheet_name = "19.2 (d)", skiprows = (0,1,2,3), usecols = "A:K")[2:45]
        df2_1 = pd.read_excel(path.join(params["datasets"], "A_Economia", "A.149.xls"), sheet_name = "cap22005c", skiprows = (0,1,2,3))[3:45]
        df2_2 = pd.read_excel(path.join(params["datasets"], "A_Economia", "A.149.xls"), sheet_name = "cap22005d", skiprows = (0,1,2,3))[2:37]
        df_3 =  pd.read_excel(path.join(params["datasets"], "A_Economia", "A.154.xls"), skiprows = (0,1,2), usecols = "A:D,F:H,J:L,N:P,R:T,V:X,Z:AB")[5:52]
