...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):
        ...
        if (column == 'pliego' and dim_type == 'temp'):
            query = 'SELECT DISTINCT month_id, sector, {}, {}_nombre, tipo_gobierno FROM temp_mef_gastos'.format(column, column)
        elif (column == 'pliego' and dim_type == 'dim'):
            query = 'SELECT * FROM temp_dim_mef_gastos_pliego'.format(column, column)
        elif column == 'ejecutora':
            query = 'SELECT DISTINCT month_id, sec_ejec AS {}, {}_nombre FROM temp_mef_gastos'.format(column, column)
        else:    
            query = 'SELECT DISTINCT month_id, {}, {}_nombre FROM temp_mef_gastos'.format(column, column)
        ...
        if column not in ['tipo_gobierno', 'pliego']:
            ...
            print('Creating temporal unique pliego id using sector and pliego ids as "str"')
...
class DimensionsPipeline(EasyPipeline):
    ...
    def steps(params):
        ...
        dtypes = {
            params.get('dimension'):                    GASTO_DTYPES_COLS[params.get('dimension')],
        ...
        if params.get('type') == 'dim':
            load_step = LoadStep('dim_mef_gastos_{}'.format(params.get('dimension')), db_connector, if_exists='drop', pk=[params.get('dimension')], dtype=dtypes)
        else:
            load_step = LoadStep('temp_dim_mef_gastos_{}'.format(params.get('dimension')), db_connector, if_exists='drop', pk=[params.get('dimension')], dtype=dtypes)
...
def run_pipeline(params: dict):
    ...
    for dimension in GASTO_DIMENSIONS_COLS:
        if dimension == "pliego":
